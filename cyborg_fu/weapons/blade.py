...
class Blade(pygame.sprite.Sprite):
    ...
    def _init_position(self, facing: Direction) -> None:
        """Set initial position and movement direction for the blade."""
        self.rect.midtop = self.position  # type: ignore[union-attr]
        self.state = State.MOVING
        if facing is Direction.RIGHT:
            self.movepos: list[int] = [40, 0]
        elif facing is Direction.LEFT:
            self.movepos = [-40, 0]
        elif facing is Direction.DOWN:
            self.image = pygame.transform.rotate(self.image, 90)  # type: ignore[arg-type]
            self.movepos = [0, 20]
    ...
    def update(self) -> None:
        """Decrease life counter and kill sprite when expired."""
        self.life -= 1
        if self.life == 0:
            self.kill()
